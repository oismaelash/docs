...
<Note>
  **Enabling calling on an official (Meta Cloud) number needs a messaging tier of `TIER_2K` (2,000 msgs/24h) or higher** — Meta's own gate for the Calling API (attempting it below that returns Meta error `138015`). Pilot Status turns calling on **automatically** when you connect the number and re-tries as the number's messaging tier climbs, so a fresh number in a low tier (e.g. `TIER_250`) simply can't take calls yet. Watch the tier badge on the **Numbers** page; once it reaches `TIER_2K`, calling is enabled on the next health poll. Unofficial (web) numbers have no such gate.
</Note>
