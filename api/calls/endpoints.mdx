...
| `POST` | `/v1/calls/{callId}/play` | **Unofficial only** | Play an audio file into an active call (server downloads it). |
...
Play a pre-recorded audio file into an active Unofficial call. The backend downloads the file (behind an SSRF guard) and streams it server-side into the call.
...
The following events fire for **both META and Unofficial** numbers: `call.ringing`, `call.connected`, `call.ended` (carries `durationSeconds` when the call was answered), `call.missed`, and `call.permission_updated`.
