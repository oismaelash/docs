...
<Frame caption="Get (or regenerate) your key on the dashboard's API Keys page — one default key per number.">
...
| **Tenant-scoped** (singleton) | The tenant (no number) | Tenant-level management: `/v1/numbers/*`, `/v1/remote-pairing/*`, `/v1/api-keys`, `/v1/branding`, `/v1/webhooks/*`, `/v1/subscription/extra-numbers`, `/v1/billing/checkout`, and `/v1/embed/sessions` (connect surface) |
