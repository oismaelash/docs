...
- **`historyImportEnabled`** — whether that history is stored at all. **It defaults to `true`**: a new number keeps the device's previous conversations. Turn it off if you do not want them.
...
<Note>
**The default has changed twice, and an existing row is never rewritten.**

| Number created | Born with |
| --- | --- |
| up to 2026-08-25 | `true` |
| 2026-08-26 to 2026-09-16 | `false` |
| from 2026-09-16 on | `true` |

Neither change touched numbers that already existed — since 25/08 the dashboard asks before connecting, so a stored `false` may be a real choice by whoever connected. **If the number predates today, read the value instead of assuming it.** The dashboard asks the question in the connect wizard, next to the phone number.
</Note>
