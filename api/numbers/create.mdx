...
Both provisioning routes on this page (`POST /v1/numbers` and `POST /v1/numbers/meta`) check capacity **before** the number row is written, and both refuse with **`402`**. The status alone does not tell you what to do — the code does:

| `error` / `code` | What it means | What fixes it |
| --- | --- | --- |
...
<Note>
  **Changed.** A plan whose allowance was simply full used to answer `INSUFFICIENT_FUNDS` as well — so an account with one number on a one-number plan was told it had no money, and went looking for a balance that was never the problem. The status is unchanged (`402`); only the code, and the fields alongside it, are new.
</Note>
