...
<Note>
  **Planned:** the stored rate already carries a `fetchedAt` (when we last pulled the card) and a `source`. Exposing both on this response is a small change and is on the list — it gives you the age of the number without anyone having to guess its validity.
</Note>
