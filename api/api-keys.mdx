...
The policy lives on the number, so you can also change it over the API with `PATCH /api/whatsapp-numbers/{id}`:

```json
{ "piiMode": "STORE_X_DAYS", "piiRetentionDays": 30 }
```

Invalid values return `400 Invalid piiMode`, or `400 piiRetentionDays must be an integer between 1 and 3650 for STORE_X_DAYS`. For the full behavior — how each mode affects the dashboard and the `/v1` read endpoints, and how redacted reads look — see [Data Retention & PII Modes](/trust/data-retention).
