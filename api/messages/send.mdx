...
<ParamField body="variables" type="object">
  Key→value map for template variables. Missing required variables produce `MISSING_TEMPLATE_VARIABLES`. For a template created in Meta's console whose dynamic URL button has a numbered placeholder (`https://ctlnk.io/{{1}}` — not the body's `{{1}}`), pass the button's URL suffix as `button_<N>`, where `N` is the button's position among the template's buttons, starting at 1: `{ "1": "Maria", "button_1": "Gggkf" }`. See [Templates](/api/templates#button-placeholders-in-templates-created-in-meta).
</ParamField>
...
<ParamField body="buttons" type="array">
  ...
  On **Meta (official) numbers**, a template's buttons are fixed by the approved template: `buttons` does not replace them and does not fill a button variable. Fill a dynamic URL button through `variables` — by its name (e.g. `link`) or, for a numbered placeholder in a template created in Meta's console, as `button_<N>`.
</ParamField>
...
<Note>
  On **Meta (official) numbers**, a template's buttons are fixed by the approved template. On a template send there, request `buttons` does not replace them and does not fill a button variable — fill a dynamic URL button through `variables`, by its name (e.g. `link`) or, for a numbered placeholder in a template created in Meta's console, as `button_<N>` (see [Templates](/api/templates#button-placeholders-in-templates-created-in-meta)).
</Note>
...
In the **Template \+ media \+ buttons** example, `buttons` does not replace the template's buttons on a Meta (official) number: there the approved template's buttons are fixed, and a dynamic URL button is filled through `variables` (see [Buttons](#buttons)).
