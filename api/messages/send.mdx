...
- On **Meta Cloud API** numbers, free-form buttons only work inside the 24-hour service window. Outside it, send an **approved template**. Template buttons use a **different vocabulary** — `QUICK_REPLY`, `URL`, `PHONE_NUMBER`, `COPY_CODE` — defined when the template is created; see [Templates](/api/templates).
