...
<Warning>
  **2 — Nothing is stored at all unless `historyImportEnabled` was `true` before the number connected.** It **defaults to `false`**. With it off, the whole `history` delivery is dropped on arrival and this endpoint answers as if the number had no past.
