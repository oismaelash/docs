...
| `FLOW_CLONE_SOURCE_NOT_ON_META` | 409 | The source Flow exists here but has not reached Meta, so there is nothing to clone from |
| `FLOW_PREVIEW_UNAVAILABLE` | 502 | Meta answered without a preview link. Not ours — retry |
| `FLOW_ENDPOINT_KEY_REQUIRES_CONFIRMATION` | 400 | Rotating over a LIVE key without `"confirm": true` |
| `FLOW_ENDPOINT_KEY_UNAVAILABLE` | 500 | The keypair could not be generated or stored |
| `FLOW_NUMBER_NOT_FOUND` | 404 | The number in the path is not this key's number |
| `FLOW_CLONE_SOURCE_NOT_ON_META` | 409 | The source exists here but has not reached Meta — nothing to clone from |
| `FLOW_PREVIEW_UNAVAILABLE` | 502 | Meta answered without a preview link. Not ours — retry |
| `FLOW_ENDPOINT_KEY_REQUIRES_CONFIRMATION` | 400 | Rotating over a LIVE key without `"confirm": true` |
| `FLOW_ENDPOINT_KEY_UNAVAILABLE` | 500 | The keypair could not be generated or stored |
| `FLOW_NUMBER_NOT_FOUND` | 404 | The number in the path is not this key's number |
