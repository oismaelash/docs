...
| `response` | nullable | the submitted answers, **already parsed** |
...
<Note>
  **`response` or `responseRaw`, never both.** Meta sends the answers as a JSON *string*. When it parses you get `response`; when it does not you get `responseRaw` with the bytes as they arrived. The submission is never dropped for being unparseable, so check which one you got.
</Note>
...
<Warning>
  **On a number configured to keep no customer content, the delivery is complete and the stored copy is not.** Your endpoint receives the full body; the payload kept in the webhook log has `from`, `response` and `responseRaw` blanked. A log entry that looks empty beside a `200` is that redaction, not a failed delivery.
</Warning>
