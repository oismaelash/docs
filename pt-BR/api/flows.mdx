...
| `FLOW_CLONE_SOURCE_NOT_ON_META` | 409 | A origem existe aqui mas não chegou à Meta — não há o que clonar |
| `FLOW_PREVIEW_UNAVAILABLE` | 502 | A Meta respondeu sem link de preview. Não é nosso — tente de novo |
| `FLOW_ENDPOINT_KEY_REQUIRES_CONFIRMATION` | 400 | Rotacionar sobre chave VIVA sem `"confirm": true` |
| `FLOW_ENDPOINT_KEY_UNAVAILABLE` | 500 | O par de chaves não pôde ser gerado ou gravado |
| `FLOW_NUMBER_NOT_FOUND` | 404 | O número do caminho não é o número desta chave |
