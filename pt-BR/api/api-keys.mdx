...
Também é configurável via `PATCH /api/whatsapp-numbers/{id}` com `{ "piiMode": "STORE_X_DAYS", "piiRetentionDays": 30 }`. Valores inválidos → `400 Invalid piiMode` ou `400 piiRetentionDays must be an integer between 1 and 3650` para `STORE_X_DAYS`. Veja a página completa em [Retenção de Dados](/pt-BR/trust/data-retention).
