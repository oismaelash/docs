...
| `STORE_X_DAYS` | Apenas conversas com atividade dentro dos últimos N dias são retornadas. O limite inferior efetivo é `max(startDate, now − piiRetentionDays)`. |
| `RELAY_ONLY` | Retorna uma lista **vazia** (`conversations: []`, `total: 0`) e inclui um campo `notice: "PII_RELAY_ONLY"`. Nenhum dado de conversa é armazenado para este número. |
