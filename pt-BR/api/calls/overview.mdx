...
<Note>
  **Habilitar chamadas em um número oficial (Meta Cloud) exige tier de mensagens `TIER_2K` (2.000 msgs/24h) ou maior** — é o próprio gate da Meta para a Calling API (tentar abaixo disso retorna o erro Meta `138015`). O Pilot Status habilita chamadas **automaticamente** ao conectar o número e re-tenta conforme o tier de mensagens sobe, então um número novo em tier baixo (ex.: `TIER_250`) simplesmente ainda não recebe chamadas. Acompanhe o selo de tier na página **Números**; ao atingir `TIER_2K`, as chamadas são habilitadas no próximo ciclo de health. Números não oficiais (web) não têm esse gate.
</Note>
