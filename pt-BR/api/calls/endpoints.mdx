...
- Números com um limite de mensagens **abaixo de 2.000/dia ainda não podem habilitar chamadas** (erro 138015 da Meta → o Pilot Status marca o número como `PENDING_TIER` e re-tenta conforme o tier sobe).
- Habilitar também exige que a WABA tenha o **campo de webhook `calls` subscrito** (ou SIP configurado); sem isso a Meta retorna **138018** e o Pilot Status marca o número como `PENDING_WEBHOOK`. Subscreva o campo `calls` na WABA (ou configure SIP) e tente de novo.
- Você também pode disparar pelo painel — o botão **Habilitar chamada** nos **detalhes Meta** do número (página Números) responde `ENABLED`, `PENDING_TIER` ou `PENDING_WEBHOOK`.
...
- Erros de chamada da Meta são encaminhados com um `code` mapeado — ex.: `META_CALL_PERMISSION_REQUIRED` (138006), limite de solicitação de permissão (138009), limite de BIC de 100 chamadas/24h (138012), chamadas não habilitadas (138000), tier de mensagens muito baixo para habilitar (138015 → `PENDING_TIER`), pré-requisitos de chamada não atendidos — subscreva o campo de webhook `calls` ou configure SIP (138018 → `PENDING_WEBHOOK`), `META_CALLING_PAYMENT_REQUIRED` (131044 — a WABA não tem método de pagamento; chamadas iniciadas pela empresa são cobradas pela Meta, adicione um cartão em Configurações de pagamento do WhatsApp Manager).