...
<Note>
  As chamadas funcionam em **dois provedores**: números **Meta Cloud API** e números **Pilot Status web** (QR/não oficial, também chamados **EVO\_GO**). Qualquer outro provedor (ex.: Evolution v2) retorna `400 FEATURE_NOT_SUPPORTED`. Todos os endpoints exigem uma chave **com escopo de número**. Veja a [visão geral](/pt-BR/api/calls/overview).
</Note>

<Warning>
  **Meta exige SDP.** No provedor Meta a mídia flui pelo navegador via WebRTC — você fornece uma oferta SDP ao iniciar e uma resposta SDP ao atender. **O EVO\_GO não usa SDP:** a mídia é tratada no **servidor** (SRTP/Opus), então `sdpOffer`/`sdpAnswer` permanecem `null` e não há modelo de permissão BIC nem superfície de configurações de chamada.
</Warning>

## Endpoints

Os provedores marcados são os que suportam cada endpoint. Chamar um endpoint em um provedor não suportado retorna `400 FEATURE_NOT_SUPPORTED`. Se um número EVO\_GO não estiver conectado, os endpoints retornam `409 WHATSAPP_INSTANCE_NOT_CONNECTED`.

| Método | Endpoint | Provedores | Descrição |
| --- | --- | --- | --- |
| `POST` | `/v1/calls` | META, EVO\_GO | Inicia uma chamada iniciada pela empresa (BIC). |
| `GET` | `/v1/calls` | META, EVO\_GO | Lista as chamadas do número (mais recentes primeiro; `limit` 1–100, `before` cursor ISO 8601). |
| `GET` | `/v1/calls/{callId}` | META, EVO\_GO | Uma chamada; adicione `?includeSdp=1` para receber `sdpOffer`/`sdpAnswer` (Meta). |
| `POST` | `/v1/calls/{callId}/accept` | META, EVO\_GO | Atende uma chamada recebida (UIC). No Meta, com resposta SDP; no EVO\_GO, sem SDP. |
| `POST` | `/v1/calls/{callId}/reject` | META, EVO\_GO | Recusa uma chamada recebida (sem corpo). |
| `POST` | `/v1/calls/{callId}/terminate` | META, EVO\_GO | Desliga uma chamada ativa (sem corpo). |
...
| `POST` | `/v1/calls/{callId}/play` | **Somente EVO\_GO** | Reproduz um arquivo de áudio na chamada (mídia no servidor). |
| `POST` | `/v1/calls/{callId}/realtime-session` | **Somente EVO\_GO** | Abre uma sessão WebSocket full-duplex de áudio PCM16. |
...
- **EVO\_GO:** não envie `sdp` — a mídia é tratada no servidor. Não há modelo de permissão BIC.
...
<Note>
  As seções 3 e 4 aplicam-se **apenas a números Meta**. No EVO\_GO não há superfície de configurações de chamada nem modelo de permissão BIC — chamar esses endpoints retorna `400 FEATURE_NOT_SUPPORTED`.
</Note>
...
## 5. Reproduzir áudio na chamada (somente EVO\_GO)

Nos números EVO\_GO a mídia é tratada no servidor, então você pode instruir o backend a **baixar um arquivo de áudio e reproduzi-lo** na chamada ativa. O backend busca a URL através de um guard de SSRF.
...
<Warning>
  Endpoint **somente EVO\_GO** — em um número Meta retorna `400 FEATURE_NOT_SUPPORTED`.
</Warning>

## 6. Sessão de áudio em tempo real (somente EVO\_GO)
...
```json
{ "wsUrl": "wss://<public-evolution-go-host>/...", "token": "...", "expiresIn": 120 }
```

<Note>
  A `wsUrl` retornada aponta para o **host público do Evolution GO** — conecte-se **diretamente** a ela (não é feito proxy). O token é de **uso único** com TTL de ~2 minutos. A camada HTTP não faz upgrade de WebSocket. Endpoint **somente EVO\_GO** (Meta → `400 FEATURE_NOT_SUPPORTED`).
</Note>
...
- Os SDPs (`sdpOffer`/`sdpAnswer`) aparecem apenas com `?includeSdp=1` em `GET /v1/calls/{callId}` — nunca na listagem. No EVO\_GO permanecem `null` (mídia no servidor).
- `durationSeconds` é definido apenas quando a chamada foi atendida.

## Webhooks

Estes eventos disparam para **ambos os provedores** (META e EVO\_GO):
...
- **EVO\_GO:** sem cobrança por minuto da Meta.
- O Pilot Status não cobra nada pelas chamadas em nenhum dos provedores.

## Erros comuns

- `400 FEATURE_NOT_SUPPORTED` — o número da chave não suporta o endpoint (ex.: provedor Evolution v2, ou um endpoint exclusivo de um provedor chamado no outro).
- `409 WHATSAPP_INSTANCE_NOT_CONNECTED` — número EVO\_GO não conectado.
