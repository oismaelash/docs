...
## DELETE /v1/subscription/extra-numbers — Remover

O On Demand é realmente pague-por-número: **você é cobrado apenas pelos números que mantém**. Há duas formas de liberar um slot pago:

- **Delete o número** (`DELETE /v1/numbers/<id>`): o slot pago é liberado **automaticamente** — os próximos meses param de cobrar aquele número. Esse é o fluxo normal.
- **Devolva um slot pré-pago** sem deletar um número (capacidade que você comprou adiantado mas não usa mais):
...
- **Sem estorno do ciclo corrente.** A remoção só para as cobranças **futuras**; a parte já paga do ciclo em andamento não é creditada de volta.
- **Não pode ficar abaixo dos números conectados.** Se a remoção deixaria o limite abaixo dos números que você ainda tem conectados, retorna **409 `NUMBER_LIMIT_EXCEEDED`** (`{ currentNumberCount, maxNumbers }`) — delete um número antes.
- **`quantity`** tem default `1` e é limitado para nunca ficar abaixo de `0` extras pagos. Chaves de escopo de número recebem **403**.

A resposta espelha o formato da compra:
