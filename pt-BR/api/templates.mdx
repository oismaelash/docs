...
`FLOW`, `LOCATION` (header ou botão), `CAROUSEL`, `CATALOG`/`MPM`, `LIMITED_TIME_OFFER`, OTP **autofill/one-tap/zero-tap**, botões `VOICE_CALL` e `ORDER_DETAILS` (botão de pagamento legado — use `PAYMENT_REQUEST`).
