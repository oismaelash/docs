...
- `number.health_*` — transições de saúde do número

**Chamadas de voz** (WhatsApp Business Calling — **somente números da Meta Cloud API**, consulte [Chamadas de Voz](/pt-BR/api/calls/overview)):
...
- `call.permission_updated` — resposta a uma solicitação de permissão de chamada (`status`: `NO_PERMISSION` | `TEMPORARY` | `PERMANENT`)
- `calls` — envelope **nativo** `calls` da Meta (bruto `entry[].changes[].value`, carregando SDPs) para sinalização WebRTC personalizada
...
Campos para `number.created` / `number.connected` / `number.disconnected` / `number.removed` (e os eventos `number.health_*`):
