...
<Note>
  Este endpoint é o jeito de ler mensagem **antiga**. O seu webhook não as replica: quando um número conecta, o histórico do aparelho é importado para o chat mas não é entregue como evento `message.received`. Veja [`PATCH /v1/numbers/{id}`](/pt-BR/api/numbers/settings) se quiser ligar essa entrega num número.
</Note>

## Coexistência: o que de fato cai aqui

Num número Meta em **coexistência**, a Meta envia o histórico do aparelho depois do pareamento, em pedaços, pelo webhook interno `history`. O que disso você consegue reler aqui é mais estreito do que o que a Meta manda, em três pontos específicos.

<Warning>
  **1 — A janela de importação é de 30 dias, não de seis meses.** Tudo o que a Meta entrega é filtrado contra `connectedAt − 30 dias` (caindo para `createdAt` em números anteriores à coluna). Mensagens mais antigas chegam e são **descartadas** — nunca são gravadas, então nenhum endpoint consegue devolvê-las depois. A Meta pode muito bem mandar seis meses; só os últimos 30 dias antes da conexão sobrevivem.
</Warning>

<Warning>
  **2 — Nada é gravado se `historyImportEnabled` não estava `true` ANTES de o número conectar.** Com ele desligado, a entrega de `history` inteira é descartada na chegada e este endpoint responde como se o número não tivesse passado.

  **Número novo nasce com `true`.** Números criados entre 2026-08-25 e 2026-09-16 nasceram com `false` e **continuam `false`** — a mudança de padrão não altera linha existente, porque desde 25/08 o painel pergunta antes de conectar e um `false` ali pode ser escolha real. Se o número é anterior, confirme o valor em vez de assumir.

  ⛔ **Não adianta ligar depois.** A Meta entrega o histórico **uma vez só**, logo após o pareamento; um `PATCH` posterior chega tarde e o que o aparelho entregou já foi descartado. Mande no `POST /v1/numbers/remote-pairing`, que aceita o bloco `settings`.

  E não dá para corrigir depois: a Meta entrega o histórico uma vez, logo após o pareamento, e não há como pedir de novo. Defina na chamada de pareamento — o `POST /v1/numbers/remote-pairing` aceita o mesmo bloco `settings` — e não num `PATCH` posterior, que corre contra a rajada. Veja [configurações do número](/pt-BR/api/numbers/settings).
</Warning>

<Tip>
  **Para diagnosticar um resultado vazio:** `GET /v1/numbers/{id}` → [`historyImport`](/pt-BR/api/numbers/list#o-bloco-historyimport) diz se o histórico chegou e foi descartado (`discarded`) ou se a Meta recusou enviá-lo (`syncRefused`).
</Tip>


<Warning>
  **3 — Num número Meta, o histórico importado nunca é entregue aos seus webhooks, nem com `webhookHistoricalMessages: true`.** Essa flag governa o trilho Evolution (QR), onde o replay realmente despacha `message.received`. O caminho `history` da Meta só persiste as linhas; ele não tem despacho de webhook. (`history` é também um dos nove campos da Meta que nunca são encaminhados nativamente, nem para uma assinatura `"*"`.)

  Ou seja: num número Meta este endpoint é a **única** forma de ler o que foi importado. Consulte-o depois do pareamento, em vez de esperar entregas que não virão.
</Warning>

<Note>
  **Contatos não têm endpoint próprio.** O que você tem é o interlocutor de cada conversa — telefone e, quando o WhatsApp forneceu, o nome de perfil — via [`GET /v1/conversations`](/pt-BR/api/conversations) e as linhas de mensagem daqui. Não existe importação de agenda separada para ler.
</Note>
