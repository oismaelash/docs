...
<Warning>
  **2 — Nada é gravado se `historyImportEnabled` não estava `true` ANTES de o número conectar.** O padrão é **`false`**. Com ele desligado, a entrega de `history` inteira é descartada na chegada e este endpoint responde como se o número não tivesse passado.
