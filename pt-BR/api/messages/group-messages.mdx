...
| Modo PII | Efeito |
| --- | --- |
| `STORE_INDEFINITE` (padrão) | Todas as mensagens de grupo são retornadas (sujeito aos filtros de data). |
| `STORE_X_DAYS` | Apenas mensagens dos últimos N dias são retornadas. O limite inferior efetivo é `max(startDate, now − piiRetentionDays)`. Mensagens fora da janela de retenção foram permanentemente excluídas pelo job diário. |
...
<ResponseField name="media" type="object | null">
  ...
  - `media.url` — URL de download direto quando disponível. Mídia inbound da **META** é espelhada no S3 (então mensagens META geralmente carregam tanto `id` **quanto** `url`); o **Evolution** fornece seu próprio link. É best-effort — pode ser `null`.
