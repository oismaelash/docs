...
<Warning>
  Um **botão de URL num carrossel de sessão é impossível** — a Meta retorna `400`. Os botões do carrossel de sessão são **somente `QUICK_REPLY`**. A mistura de `QUICK_REPLY` \+ `URL` num mesmo cartão é permitida **apenas** num carrossel de *template*.
</Warning>
...
- **`hasBody` é tudo ou nada**: se `cardTemplate.hasBody` for `true`, todo cartão precisa de um `body`; se `false`, nenhum pode ter. Os `buttons` de cada cartão devem corresponder a `cardTemplate.buttons` em **quantidade, tipo e ordem** (todos `QUICK_REPLY`).
