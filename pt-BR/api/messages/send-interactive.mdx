...
<Warning>
  **Em números Meta Cloud API, não misture `url` com `reply` num envio FREE-FORM.**
  O WhatsApp não tem forma suportada de combinar botão de link com botões de resposta
  fora de um template — o CTA URL é um tipo próprio de mensagem interativa, e leva um
  único botão.

  As duas ordens falham, e uma delas falha *em silêncio*:

  | você envia | o que acontece |
  | --- | --- |
  | `reply` antes de `url` | recusado na hora — `Invalid mixed buttons sequence` |
  | `url` antes de `reply` | **aceito com `202`, e nunca entregue** (`Message undeliverable`) |

  Medido em 15/08/2026 na mesma conversa, uma variável por vez: texto puro entregou,
  só `reply` entregou, só `url` foi lido — só a mistura falhou.

  **O que fazer:** mande duas mensagens (uma com o botão de link, outra com os de
  resposta), ou use um **template** aprovado, onde o conjunto de botões é validado na
  aprovação. A restrição não vale para números não oficiais (Pilot Status web).
</Warning>
