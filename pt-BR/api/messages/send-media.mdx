...
<ParamField body="media" type="string" required>
  Uma **URL http(s) pública** ou um **data URI base64** (ex.: `data:image/png;base64,...`). Base64 funciona em números **Meta Cloud API**; números **não oficiais (Pilot Status web)** exigem uma URL pública. O base64 vai até **50 MiB** decodificados para `document` e **16 MiB** para os demais tipos; acima disso a requisição é recusada com `400` — envie um arquivo maior por URL.
</ParamField>
