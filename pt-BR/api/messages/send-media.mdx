...
- `sticker` precisa ser **`image/webp`** — estática até **100 KB**, animada até **500 KB**. Outro formato é recusado pelo WhatsApp, não por esta API: a falha chega no webhook `message.failed`, não na resposta 202. Figurinha não aceita `caption` nem `mediaFilename` (os dois retornam 400) e não pode ser enviada como mídia de template.
