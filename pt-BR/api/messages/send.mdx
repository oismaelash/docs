...
<ParamField body="media" type="string">
  Uma URL http(s) pública **ou** um data URI base64 (ex.: `data:audio/ogg;base64,AAAA...`) para um arquivo de imagem, vídeo, documento ou áudio. Base64 é aceito para **todos** os tipos de mídia em números Meta Cloud API. Em números **não oficiais (Pilot Status web)**, base64 **não** é aceito — use uma URL http(s) pública. Sobrescreve qualquer `mediaUrl` embutido no template. O base64 tem teto pelo tamanho decodificado: **50 MiB** (52.428.800 bytes) quando `mediaType` é `document`, **16 MiB** (16.777.216 bytes) nos demais tipos e quando `mediaType` é omitido. Acima do teto a requisição é recusada com `400 Validation error`, e `details.fieldErrors.media` informa o limite. Envie um arquivo maior como URL http(s) pública — em números Meta Cloud API ele passa a seguir os tetos da própria Meta (documento 100 MB, vídeo e áudio 16 MB, imagem 5 MB), e um arquivo acima deles falha depois do `202` com `META_MEDIA_TOO_LARGE`.
</ParamField>
