...
<ParamField body="buttons" type="array">
  ...
  Em **números Meta (oficiais)**, os botões de um template são fixados pelo template aprovado: `buttons` não os substitui e não preenche variável de botão. Preencha um botão de URL dinâmica por `variables` — pelo nome dele (ex.: `link`) ou, para placeholder numerado em template criado no painel da Meta, como `button_<N>`.
</ParamField>
...
No exemplo **Template \+ mídia \+ botões**, o `buttons` não substitui os botões do template num número Meta (oficial): ali os botões do template aprovado são fixos, e um botão de URL dinâmica é preenchido por `variables` (veja [Botões](#botões)).
