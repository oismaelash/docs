...
<ParamField body="media" type="string">
  Uma URL http(s) pública **ou** um data URI base64 (ex.: `data:audio/ogg;base64,AAAA...`) para um arquivo de imagem, vídeo, documento ou áudio. Base64 é aceito para **todos** os tipos de mídia na Meta Cloud API e na Evolution v2. Em números **Evolution GO**, base64 **não** é aceito — use uma URL http(s) pública. Sobrescreve qualquer `mediaUrl` embutido no template.
</ParamField>

<ParamField body="mediaType" type="string">
  `image`, `video`, `document` ou `audio`. Defina explicitamente quando a extensão da URL não for óbvia (ex.: PDFs cuja URL não termina em `.pdf`). Quando `mediaType` é `audio`, o arquivo é entregue como uma **nota de voz (PTT)** do WhatsApp na Meta Cloud API, Evolution v2 e Evolution GO. Na Evolution v2 e Evolution GO, um indicador de presença "gravando áudio" é exibido logo antes da entrega; a Meta Cloud API não possui API de presença de saída, então nenhum indicador é exibido para envios de áudio pela Meta.
</ParamField>
...
<Note>
  Enviar com `media` requer uma assinatura paga ativa. Caso contrário, a API retorna **402** com `code: SUBSCRIPTION_REQUIRED_FOR_MEDIA`.
</Note>
...
<ParamField body="buttons" type="array">
  ...
  `buttons` é incompatível com `mediaType: "video"` ou `mediaType: "document"` (o WhatsApp não suporta botões com vídeo/PDF).
</ParamField>
...
| Status | Significado |
| --- | --- |
| `400` | Payload inválido (ex.: telefone fora do E.164, campos obrigatórios ausentes, `MISSING_TEMPLATE_VARIABLES`) |
| `401` | Header de chave de API ausente/inválido (`x-api-key` / `x-api-key-id`) |
| `402` | Assinatura paga necessária para enviar mídia (`code: SUBSCRIPTION_REQUIRED_FOR_MEDIA`) |
| `403` | Operação bloqueada (ex.: texto livre não suportado pelo provedor: `code: FREE_FORM_NOT_SUPPORTED`; ou uma chave com escopo de tenant em um endpoint com escopo de número) |
