...
<ParamField body="buttons" type="array">
  ...
  `buttons` pode ser combinado com qualquer `mediaType`. A API não rejeita botões com `mediaType: "video"` ou `mediaType: "document"` (a Evolution GO e a Evolution v2 os aceitam); observe que a Meta Cloud API pode rejeitar algumas combinações de botões com vídeo/documento no momento da entrega.
</ParamField>
