...
- Em números **Meta Cloud API**, botões de texto livre só funcionam dentro da janela de atendimento de 24 horas. Fora dela, envie um **template aprovado**. Botões de template usam um **vocabulário diferente** — `QUICK_REPLY`, `URL`, `PHONE_NUMBER`, `COPY_CODE` — definidos na criação do template; veja [Templates](/pt-BR/api/templates).
