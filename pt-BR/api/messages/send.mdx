...
<Frame caption="A página Mensagem usa este mesmo endpoint — escolha um template, mapeie variáveis para colunas da planilha, suba destinatários e copie o curl gerado automaticamente.">
  ![Página Mensagem do painel com seletor de template e pré-visualização, variáveis mapeadas para colunas da planilha, lista de destinatários com upload de Excel/CSV, ações Enviar e Agendar e o curl gerado automaticamente da chamada exata da API](/images/dashboard/message-composer.png)
</Frame>
...
<ParamField body="variables" type="object">
  Mapa chave→valor para as variáveis do template. Variáveis obrigatórias ausentes produzem `MISSING_TEMPLATE_VARIABLES`. Para um template criado no painel da Meta cujo botão de URL dinâmica tem um placeholder numerado (`https://ctlnk.io/{{1}}` — que não é o `{{1}}` do body), passe o sufixo da URL do botão como `button_<N>`, em que `N` é a posição do botão entre os botões do template, começando em 1: `{ "1": "Maria", "button_1": "Gggkf" }`. Veja [Templates](/pt-BR/api/templates#placeholders-de-botão-em-templates-criados-na-meta). Num número oficial, a mesma chave também leva o código de um botão de copiar código cujo valor não tem `{{variável}}` — `{ "button_2": "BLACKFRIDAY" }`; veja [Botões de copiar código em números oficiais](/pt-BR/api/templates#botões-de-copiar-código-em-números-oficiais).
</ParamField>
...
<ParamField body="media" type="string">
  Uma URL http(s) pública **ou** um data URI base64 (ex.: `data:audio/ogg;base64,AAAA...`) para um arquivo de imagem, vídeo, documento ou áudio. Data URIs base64 são aceitos em números Meta Cloud API e em números **não oficiais (Pilot Status web)**. Em números não oficiais, a Pilot Status sobe o arquivo decodificado para o próprio armazenamento e envia uma URL ao provedor; se esse upload falhar, a mensagem falha com `MEDIA_REHOST_FAILED`. Para figurinhas em números não oficiais, use uma URL http(s) pública. Sobrescreve qualquer `mediaUrl` embutido no template. O base64 tem teto pelo tamanho decodificado: **50 MiB** (52.428.800 bytes) quando `mediaType` é `document`, **16 MiB** (16.777.216 bytes) nos demais tipos e quando `mediaType` é omitido. Acima do teto a requisição é recusada com `400 Validation error`, e `details.fieldErrors.media` informa o limite. Envie um arquivo maior como URL http(s) pública — em números Meta Cloud API ele passa a seguir os tetos da própria Meta (documento 100 MB, vídeo e áudio 16 MB, imagem 5 MB), e um arquivo acima deles falha depois do `202` com `META_MEDIA_TOO_LARGE`.
</ParamField>

<ParamField body="mediaType" type="string">
  `image`, `video`, `document`, `audio`, `voice` ou `sticker`. Defina explicitamente quando a extensão da URL não for óbvia (ex.: PDFs cuja URL não termina em `.pdf`). Quando `mediaType` é `audio`, o arquivo é entregue como uma **nota de voz (PTT)** do WhatsApp em todos os provedores. Um indicador de presença é exibido ao contato logo antes da entrega: números não oficiais (Pilot Status web) mostram "gravando áudio"; números Meta Cloud API mostram "digitando" — a Cloud API não tem variante de gravação, e ele só aparece quando a conversa tem uma mensagem recebida recente para ancorar. O "digitando" é mostrado antes de todo envio conversacional em números Meta e Evolution; você também pode dispará-lo de forma independente via [Indicador de Digitando](/pt-BR/api/messages/typing). `sticker` envia uma **figurinha** do WhatsApp — só em envio direto de mídia, nunca com `templateId`, nunca com `caption`, e o arquivo precisa ser **`image/webp`** com **exatamente 512×512 pixels** (estática até **100 KB**, animada até **500 KB**).
</ParamField>
...
<ParamField body="buttons" type="array">
  Até 3 botões que **sobrescrevem** os botões do template (exceto em números Meta — veja abaixo). Cada botão possui `type` e `displayText` mais campos específicos do tipo:
  ...
  Em **números Meta (oficiais)**, os botões de um template são fixados pelo template aprovado: `buttons` não os substitui e não preenche variável de botão. Preencha um botão de URL dinâmica por `variables` — pelo nome dele (ex.: `link`) ou, para placeholder numerado em template criado no painel da Meta, como `button_<N>`. O código de um botão de copiar código também vai em `variables` como `button_<N>`, não em `copyCode`.
</ParamField>
...
<ParamField body="list" type="object">
  Lista interativa de seleção única para um envio de texto livre. Requer `text`; exclusiva com `templateId`, `buttons`, `header` e mídia direta (`footer` é permitido). Veja [Listas](#listas) abaixo.
</ParamField>
...
- Em números Meta, mensagens de texto livre só funcionam dentro da **janela de conversa de 24h** do WhatsApp. Fora da janela, um erro `META_OUTSIDE_24H_WINDOW` é retornado no webhook `message.failed` — use um template aprovado em vez disso. Verifique se a janela está aberta antes de enviar com [`GET /v1/service-window`](/pt-BR/playground/messages/janela-de-atendimento).

## Botões

`buttons` adiciona até **3** botões interativos — em um envio de **texto livre**, ou para **sobrescrever** os botões de um template em um envio de **template** (não em números Meta — veja a nota abaixo). Cada item precisa de `type` \+ `displayText` e um campo específico do tipo:

| `type` | Campo extra | Comportamento |
| --- | --- | --- |
| `reply` | `id` (string) | Resposta rápida. O toque chega como webhook de entrada normal cujo `content` é o rótulo do botão — o `id`**não** é devolvido. |
| `url` | `url` (https) | Abre um link. |
| `call` | `phoneNumber` (E.164) | Liga para o número. |
| `copy` | `copyCode` (string) | Copia um código para a área de transferência. |

```json
{
  "text": "Confirmar seu agendamento?",
  "destinationNumber": "+5511999999999",
  "buttons": [
    { "type": "reply", "displayText": "Confirmar", "id": "confirmar" },
    { "type": "url", "displayText": "Detalhes", "url": "https://exemplo.com/pedido/123" }
  ]
}
```
...
- Em números **Meta Cloud API**, botões de texto livre só funcionam dentro da janela de atendimento de 24 horas. Fora dela, envie um **template aprovado**. Botões de template usam um **vocabulário diferente** — `QUICK_REPLY`, `URL`, `PHONE_NUMBER`, `COPY_CODE`, `PAYMENT_REQUEST` — definidos na criação do template; veja [Templates](/pt-BR/api/templates).

<Note>
  Em **números Meta (oficiais)**, os botões de um template são fixados pelo template aprovado. Num envio de template ali, o `buttons` da requisição não os substitui e não preenche variável de botão — preencha um botão de URL dinâmica por `variables`, pelo nome dele (ex.: `link`) ou, para placeholder numerado em template criado no painel da Meta, como `button_<N>` (veja [Templates](/pt-BR/api/templates#placeholders-de-botão-em-templates-criados-na-meta)). O código de um botão de copiar código segue o mesmo caminho, como `button_<N>` — não em `copyCode` (veja [Botões de copiar código em números oficiais](/pt-BR/api/templates#botões-de-copiar-código-em-números-oficiais)).
</Note>
    ...
    "sections": [
      {
        "title": "Atendimento",
        "rows": [
          { "id": "clientes", "title": "Clientes", "description": "Consultar e cadastrar clientes" },
          { "title": "Pedidos", "description": "Acompanhar um pedido existente" }
        ]
      }
    ]
  }
...
No exemplo **Template \+ mídia \+ botões**, o `buttons` não substitui os botões do template num número Meta (oficial): ali os botões do template aprovado são fixos, e um botão de URL dinâmica ou o código de um botão de copiar código é preenchido por `variables` (veja [Botões](#botões)).
...
| Status | Significado |
| --- | --- |
| `400` | Payload inválido (ex.: telefone fora do E.164, campos obrigatórios ausentes, `MISSING_TEMPLATE_VARIABLES` / `INVALID_TEMPLATE_VARIABLE_VALUE` nomeando a chave, ou texto livre não suportado pelo provedor: `code: FREE_FORM_NOT_SUPPORTED`), ou, num número oficial, template não aprovado na Meta: `code: META_TEMPLATE_NOT_APPROVED` |
...
Falhas de entrega assíncronas (ex.: `META_OUTSIDE_24H_WINDOW`, `META_TEMPLATE_PAUSED`, `WHATSAPP_NOT_EXIST`) aparecem como `errorCode` em [`GET /v1/messages/{id}`](/pt-BR/api/messages/status) e [`GET /v1/messages`](/pt-BR/api/messages/list), nos Logs, e via webhook `message.failed` em números não oficiais — veja [Códigos de erro de log](/pt-BR/api/messages/log-error-codes).