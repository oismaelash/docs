...
- **`historyImportEnabled`** — se esse histórico é armazenado. **O padrão é `true`**: número novo guarda as conversas anteriores do aparelho. Desligue se não as quiser.
...
<Note>
**O padrão já mudou duas vezes, e linha existente nunca é reescrita.**

| Número criado | Nasceu com |
| --- | --- |
| até 2026-08-25 | `true` |
| 2026-08-26 a 2026-09-16 | `false` |
| de 2026-09-16 em diante | `true` |

Nenhuma das mudanças tocou em números já criados — desde 25/08 o painel pergunta antes de conectar, e um `false` gravado pode ser escolha real de quem conectou. **Se o número é anterior a hoje, leia o valor em vez de o assumir.** No painel, a pergunta aparece no assistente de conexão, ao lado do telefone.
</Note>
