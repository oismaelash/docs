...
| --- | --- | --- |
| Conecta num Chatwoot **compartilhado** que já tem outros números Meta **diretos** | **Espelho via API (A)** | O nativo mudaria uma configuração **global** do Chatwoot e derrubaria esses números (a camada só aceita chave `ps_`). |
| Usa um número **não oficial** (web/QR) | **Espelho via API (A)** | Mensagem de não oficial só desce por espelho; o nativo aqui serviria só para voz (e ainda exige Chatwoot dedicado). |
| Tem Chatwoot **dedicado** \+ número **oficial** \+ precisa de **voz/chamada nativa** | **Nativo Cloud (B)** | Único modo com chamada nativa; a mensagem fica nativa (sem espelho = menos partes móveis). |
| Tem Chatwoot **dedicado** \+ número oficial, **só chat** | Qualquer um — **Automático** leva ao nativo | Fique no espelho se o número puder ir para um Chatwoot compartilhado depois (evita retrabalho). |
| Quer o **menor atrito**, sem restrição de instância | **Automático** | Acerta o caso comum: oficial → nativo, não oficial → espelho. |
...
## Enviar um template pelo Chatwoot

Numa inbox de **espelho via API (Modelo A)**, a resposta do atendente sai como um template aprovado — o jeito de reabrir uma conversa num número oficial fora da janela de 24 horas da Meta — quando pede um de uma destas duas formas:

- **O comando `/template`.** Digite `/template <nome> [valor1, valor2, ...]` no compositor: os valores separados por vírgula preenchem o `{{1}}`, `{{2}}`, … do template, em ordem. Um argumento escrito `button_<N>=valor` preenche uma variável de botão — um placeholder numerado de URL ou, num número oficial, o código de um botão de copiar código —, em que `N` é a posição do botão na lista de botões do template, começando em 1, e todos os botões contam (veja [Templates](/pt-BR/api/templates#placeholders-de-botão-em-templates-criados-na-meta)):

  ```text
  /template link_pagamento_pix Maria, R$ 10, 21/09/2026, button_1=SdjLn
  ```

  Um argumento `button_<N>=` não consome um número posicional: `Ana, button_2=SdjLn, 50` preenche `{"1": "Ana", "2": "50", "button_2": "SdjLn"}`. Os valores não podem conter vírgula — `R$ 10,00` viraria dois valores (`R$ 10` e `00`). Qualquer outro texto `chave=valor` continua sendo um valor posicional comum.
- **Os parâmetros de template nativos do Chatwoot.** Templates enviados com o `template_params` do próprio Chatwoot são atendidos, inclusive no formato estruturado que o seletor de templates de WhatsApp dele monta (`processed_params` com `body` e `buttons`): os parâmetros do body preenchem as variáveis do template, e os parâmetros de botão (URL e copiar código) viram `button_<N>` pela posição.

## Submissões de Flow chegam como nota privada

Um [Flow do WhatsApp](/pt-BR/concepts/flows) enviado carrega as respostas em `interactive.nfm_reply`, e **nenhum dos dois modos de canal as mostra na bolha**:

- **Espelho via API (Modelo A)** — o Pilot Status está no caminho, então corrige as duas metades: a bolha pública é substituída por uma linha fixa curta (**`📋 Formulário respondido`**) no lugar do rótulo da Meta, e as respostas são publicadas como **nota privada** na mesma conversa — uma linha `campo: valor` por resposta. Os atendentes leem no mesmo tópico; o cliente nunca vê a nota.
- **Nativo Cloud (Modelo B)** — o Chatwoot lê o webhook da Meta por conta própria e não tem parser para `nfm_reply`, então a mensagem cai com **`content: null`**. É uma limitação do Chatwoot; nenhuma configuração daqui muda o que o parser dele entende. O Pilot Status **não** espelha essas conversas (espelhar duplicaria toda mensagem), então nesse modo não há nota privada e não há nada que a gente consiga acrescentar ao tópico.

<Note>
  **O Modelo B tem uma forma de ler as respostas, e o Modelo A tem uma melhor que a nota:** assine o evento de webhook `flow.response_received`. A nota é uma conveniência para humanos e corta cada valor em 500 caracteres; o evento carrega a submissão inteira, já parseada. Veja [Receber as respostas de um Flow](/pt-BR/guides/flow-responses).
</Note>

<Note>
  Uma nota que falha ao ser publicada não é retentada — a bolha e a resposta guardada já estão lá, e repetir o espelho mostraria ao atendente o mesmo formulário duas vezes. O evento é a cópia que retenta.
</Note>

## O que o atendente vê no Chatwoot

- **Resposta que a Pilot Status recusa fica marcada como falha, com o motivo.** Quando uma resposta escrita no Chatwoot não pode ser enviada — o WhatsApp está recusando envios do número, a cobrança está suspensa, a janela de 24h está fechada num número oficial, o `/template` não existe, o anexo não tem URL pública —, a mensagem fica marcada como falha no Chatwoot, com o motivo na bolha, e uma nota privada na conversa explica o porquê. Ela não é retentada. A mesma falha aparece nos Logs do número, com o seu [código de erro](/pt-BR/api/messages/log-error-codes#respostas-de-atendentes-no-chatwoot).
- **Nativo Cloud (Modelo B): mensagens enviadas pela API da Pilot Status caem na conversa aberta do contato.** O Chatwoot abre uma conversa nova para o contato cada vez que a anterior é resolvida; a mensagem da API vai atrás dela. Se não houver conversa aberta, a Pilot Status abre uma, e a resposta do cliente entra nela.
- **Nativo Cloud (Modelo B): "Esta mensagem não é suportada. Você pode visualizar esta mensagem no aplicativo do WhatsApp."** Em números em coexistência, o WhatsApp às vezes entrega um aviso provisório (erro `131060`, "This message is unavailable") antes da mensagem real, com o mesmo id — mais comum na primeira mensagem de um contato novo ou depois de um longo silêncio. O Chatwoot fica com o primeiro que recebe, então a Pilot Status segura o aviso por alguns segundos e o descarta quando a mensagem real chega. Quando o WhatsApp nunca entrega o conteúdo para a API, o aviso é entregue como está: a mensagem existe só no aplicativo do WhatsApp, e a bolha é o sinal para conferir lá.
- **Espelho API (Modelo A): mídia que ainda não está disponível.** O espelho espera até 20 segundos o arquivo recebido ser guardado e então posta a bolha com o anexo. Se ele ainda não estiver disponível, a bolha diz `[mídia]` seguido de um link, e o link abre o arquivo assim que ele é guardado. Um arquivo que chega sem nome recebe o nome do tipo e o id da mensagem — por exemplo `imagem-3EB03F18.jpg`.
