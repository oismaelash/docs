...
- **Só coexistência \+ espelho via API.** O botão aparece só para números Meta de coexistência com Chatwoot conectado no modo **espelho via API (Modelo A)**. Uma inbox nativa `whatsapp_cloud` não é elegível.
...
- **Ao final**, as conversas tocadas são marcadas como lidas e resolvidas para não gerar uma rajada de notificações aos atendentes.
