...
description: "Conecte agentes de IA ao WhatsApp através do servidor MCP hospedado da Pilot Status: autenticação OAuth ou x-api-key, 54 ferramentas sobre a API pública /v1, concessões por número."
---
...
| Chamadas de voz (apenas Meta) | `call_initiate`, `calls_list`, `call_get`, `call_accept`, `call_reject`, `call_terminate`, `call_settings_get`/`update`, `call_permission_get`/`request` |
