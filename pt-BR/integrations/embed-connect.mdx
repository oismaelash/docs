...
<Steps>
  ...
  <Step title="O frontend incorpora">
    ...
    `ConnectOptions`: `token` (obrigatório), `baseUrl?` (padrão `https://connect.pilotstatus.com.br`), `mode?` (`"page"` | `"button"` — omita para automático, veja [Modo botão](#modo-botão)), `onPaired(d)`, `onError(e)`, `onExpired()`.
  </Step>
</Steps>
...
A aparência do botão é definida **na hora de gerar o link**, dentro de `branding.button`, e viaja no token assinado. Mandar esse objeto também põe o link em [modo botão](#modo-botão) por padrão, então esse único objeto é o pedido inteiro: _estilize um botão e receba um botão._
