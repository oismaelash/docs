...
| `PILOT_STATUS` (pareamento por QR, padrão) | UUID puro | **24 horas** | `provider`, `instance`, `remotePairingUrl`, `maskedNumber` e `warnings` quando há algo a relatar |
...
No modo botão a página também limpa o próprio fundo, então **a sua página aparece através do iframe** e o botão pousa direto no seu layout, em vez de ficar sobre um retângulo sólido perdido ali. Nada a configurar do seu lado.
