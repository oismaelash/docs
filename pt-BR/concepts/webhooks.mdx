...
<Warning>
  **Mudança incompatível — `data.numberId` é sempre o ID do número.** Em todo evento `number.*`, `data.numberId` agora é o ID do **número** de WhatsApp (o mesmo `id` retornado por `GET /v1/numbers`). Antes, `number.created`, `number.connected` e `number.removed` carregavam o ID interno da **instância**, enquanto os eventos de saúde já carregavam o ID do número. **Se o seu consumidor casava os eventos de ciclo de vida pelo ID da instância, ele vai parar de casar — passe a usar o ID do número.**
</Warning>
