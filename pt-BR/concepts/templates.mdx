...
Após salvar, `GET /v1/templates/{id}` retorna todas as chaves de variáveis detectadas no array `variables` de nível superior (plano). Para um modelo criado no painel da Meta com marcador numerado em botão, o array também lista a chave `button_<N>` do botão (ex.: `["1", "2", "3", "button_1"]`) — forneça no envio todas as chaves que ele lista.
