...
<Note>
  ...
  Esta página é o contrato requisição por requisição. Para o mesmo recurso em linguagem de produto — o que configurar, em que ordem, e o que uma pessoa vê quando está errado — leia [Flows com dados ao vivo](/pt-BR/guides/flow-live-data).
</Note>
...
<Note>
  **Um arquivo escolhido num `PhotoPicker` ou `DocumentPicker` chega até nós NESTA troca, e nunca no `nfm_reply`.** É por isso que um Flow sem endpoint não recebe anexo nenhum. O Pilot Status re-hospeda o arquivo e o publica na submissão como `flowMedia` / `media` — o evento é emitido assim que a submissão é gravada e a re-hospedagem roda depois dele, então o `status` normalmente vem `PENDING` quando o evento cai, e o `flowMedia[].url` é um link que já funciona: buscá-lo espera a transferência em vez de dar 404. O formato, e o que fazer com isso, estão em [Receber as respostas de um Flow](/pt-BR/guides/flow-responses). Os limites da própria Meta (um picker por tela, 25 MiB, 20 dias no CDN dela) estão em [Flows](/pt-BR/concepts/flows).
</Note>
...
<Warning>
  **O que volta na submissão é só o que você nomeou em `params`, e as respostas não estão ali.** A Meta desembrulha esse objeto no `nfm_reply`, então, num Flow `data_exchange`, o `data.response` publica exatamente o formato acima e mais nada — `{"flow_token": "a1b2c3..."}` — enquanto a mesma submissão já tinha entregado `{"observacao": "Aobaaa", "foto": [ … ]}` ao seu endpoint. As respostas foram para **você**, tela a tela. Elas nunca voltaram pela Meta.

  Leia-as em **`data.submitted`**, que carrega as requisições `data_exchange` que encaminhamos, cada uma com a sua tela e o seu horário — veja [Receber as respostas de um Flow](/pt-BR/guides/flow-responses). Um Flow comum, sem endpoint, é o outro caso: nada recolheu as respostas no caminho, então ali o `response` é onde estão todas elas.
</Warning>
...
A tela do Flow no painel lista as trocas recentes: a ação, a tela, o desfecho, o seu status HTTP e quanto tempo você levou. Os corpos são gravados **só em caso de falha** — uma troca bem-sucedida guarda tempo e formato e nenhum dos dois corpos, nem o que encaminhamos nem o que você respondeu — e o registro inteiro expira em 24 horas.

<Warning>
  **"Não está neste registro" não é "não foi guardado".** O que o cliente digitou numa tela `data_exchange` é guardado e publicado: é o `data.submitted` da submissão, com cada entrada carregando a tela em que foi preenchida e o momento em que aconteceu, no evento `flow.response_received` e no `GET /v1/flows/{id}/responses`, por 31 dias.

  Um dia a mais que a própria resposta, de propósito. A troca é capturada enquanto a pessoa ainda está preenchendo, minutos antes da submissão que cria a linha da resposta — com uma janela igual, as trocas expirariam primeiro e os últimos minutos de uma resposta publicariam uma lista vazia. O formato completo está em [Receber as respostas de um Flow](/pt-BR/guides/flow-responses).
</Warning>

<Note>
  **O `submitted` é legitimamente mais curto que esta lista, e isso não é falha.** Só as trocas `data_exchange` são capturadas. O `INIT` carrega o estado que *nós* semeamos — os params com que o Flow foi enviado, devolvidos de volta — e o `BACK` é navegação; nenhum dos dois é uma resposta, então nenhum é guardado. Conte as linhas daqui e as entradas de lá: os dois números não batem.

  Ele também só acrescenta, nunca reescreve e nunca deduplica: quem volta uma tela e a envia de novo produz uma segunda entrada, genuína, e é o horário que separa as duas passagens sob um mesmo `flow_token`. E é a requisição que **a Meta manda para nós** — nunca o que o seu endpoint respondeu, e é por isso que `extension_message_response` continua não aparecendo em lado nenhum da Pilot Status.
</Note>
...
<Steps>
  ...
  <Step title="3. `endpoint_uri` — na Meta, no Flow">
    A Meta só chama um endpoint que ela tem registrado, e **salvar a URL no passo 2 não registra nada na Meta**. O registro é uma requisição própria — `POST /v1/flows/{flowId}/endpoint` — porque fazê-lo como efeito colateral de um rename sequestraria o Flow de quem já roda um endpoint próprio. Enquanto não for feito, a Meta se recusa a publicar o Flow.
  </Step>
</Steps>
...
| **`endpoint_uri`** — para onde a Meta chama | Por **Flow**, guardado **na Meta** | `POST /v1/flows/{flowId}/endpoint`, ou **Registar na Meta** na tela de **Flows** do painel |
...
### Passo 3 — registre a gente na Meta

A Meta só chama um endpoint que ela tem registrado no Flow. O passo 2 não registrou nada — esta é a requisição que registra:
...
Ela escreve `endpointUri` — a nossa URL de proxy para este Flow — no `endpoint_uri` do Flow na Meta, e responde com o mesmo estado que o `GET` devolve:
...
O corpo é opcional, e `confirmOverwrite` (booleano) é o único campo aceito — o número vem da chave, como em todo o `/v1`. Não há `secret` nesta resposta: o `POST` nunca cria nenhum.

<Note>
  O painel faz o mesmo ato. Na tela de **Flows**, em **Apontar a Meta para cá**, o botão **Registar na Meta** chama esta rota; um `409` abre uma confirmação mostrando o endereço que você está prestes a deslocar, e `confirmOverwrite` é o que o botão **Assumir mesmo assim** envia.
</Note>

<Warning>
  **Sem esta chamada, um Flow `data_exchange` não pode nem ser publicado.** A Meta recusa o `publish` com *"Publishing without specifying 'endpoint_uri' is forbidden"*, e nada mais no Pilot Status escreve esse campo.
</Warning>

<Note>
  **Por que é uma segunda requisição e não uma flag na gravação.** O `PUT` é você descrevendo o **seu** webhook; o `POST` é a gente dizendo à **Meta** para rotear o Flow por nós. Fundidos num ato só, um cliente que roda o próprio endpoint de Flow perderia todas as trocas na próxima vez que corrigisse um erro de digitação numa URL — o rename re-apontaria o Flow em silêncio. Eles são separados para que re-apontar seja algo que você faz de propósito.
</Note>

**É idempotente.** Um Flow cujo `endpoint_uri` já é o nosso responde `200` e não escreve nada na Meta. A verificação lê a Meta **ao vivo**, não a coluna `metaEndpointUri` sincronizada de hora em hora — essa coluna é muda sobre uma URI registrada na última hora, que é exatamente a janela em que duas pessoas configuram o mesmo Flow.

<Warning>
  **`409 FLOW_ENDPOINT_URI_WOULD_OVERWRITE` — a Meta já aponta este Flow para outro lugar.** A resposta nomeia esse outro lugar, e só ele:
  ...
  Reenviar a mesma requisição com `{"confirmOverwrite": true}` assume o Flow — e **assumir tira TODAS as trocas deste Flow de quem responde por ele hoje**. A escrita da Meta é uma sobrescrita simples: sem mesclagem, sem desfazer, e nada avisa o endpoint deslocado de que ele parou de ser chamado. Descubra o que está do outro lado de `currentEndpointUri` antes.

  A nossa URL deliberadamente **não** é ecoada de volta: ela carrega o token de endpoint do número, e esse token é a credencial do proxy. `currentEndpointUri` é o deles, que é o que você precisa para reconhecê-los.
</Warning>

<Note>
  **`422 FLOW_ENDPOINT_NUMBER_HAS_NO_KEY`** — o número não tem chave de endpoint de Flow, então não existe URL para registrar. Gere a chave (passo 1) e registre de novo.
</Note>

Leia o endpoint de volta quando quiser:
...
`FLOW_ENDPOINT_URI_WOULD_OVERWRITE` é a única falha daqui que acrescenta uma quarta chave, `currentEndpointUri`.
...
| `FLOW_ENDPOINT_CONFIRM_INVALID` | 400 | Só no `POST`. `confirmOverwrite` não é booleano. `"false"` é uma **string** verdadeira, e é recusada em vez de lida como consentimento. |
| `FLOW_ENDPOINT_URI_WOULD_OVERWRITE` | 409 | Só no `POST`. A Meta já aponta este Flow para outro endpoint, nomeado em `currentEndpointUri`. `409` e não `400`: não há nada de malformado na requisição — o que bloqueia é o estado na Meta. |
| `FLOW_ENDPOINT_NUMBER_HAS_NO_KEY` | 422 | Só no `POST`. O número não tem chave de endpoint, então não há URL para registrar. Gere a chave primeiro. |

<Note>
  **`FLOW_ENDPOINT_NUMBER_HAS_NO_KEY` é a única grafia que aparece duas vezes** — como item de `warnings` numa leitura ou gravação, e como `422` no registro. É o mesmo fato em dois papéis: salvar um destino num número sem chave pode dar certo *com aviso*, e registrar na Meta não pode dar certo de jeito nenhum.
</Note>
...
- [Flows com dados ao vivo](/pt-BR/guides/flow-live-data) — a mesma configuração, sem o contrato HTTP
