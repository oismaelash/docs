...
<Note>
  URIs de dados Base64 são aceitos em números Meta Cloud API; números **não oficiais (Pilot Status web)** exigem uma URL http(s) pública. O base64 vai até 50 MiB decodificados para `document` e 16 MiB para os demais tipos — envie arquivos maiores por URL.
</Note>
