...
<Note>
  URIs de dados Base64 são aceitos em números Meta Cloud API e em números **não oficiais (Pilot Status web)**. Em números não oficiais, a Pilot Status sobe o arquivo decodificado para o próprio armazenamento e envia uma URL ao provedor; se esse upload falhar, a mensagem falha com `MEDIA_REHOST_FAILED`. Para figurinhas em números não oficiais, use uma URL http(s) pública. O base64 vai até 50 MiB decodificados para `document` e 16 MiB para os demais tipos — envie arquivos maiores por URL.
</Note>
...
- Consulte [`GET /v1/messages/{id}`](/pt-BR/api/messages/status) para `QUEUED` → `SENT` → `DELIVERED` → `READ` → `PLAYED` (ou `FAILED` / `CANCELED`), ou
