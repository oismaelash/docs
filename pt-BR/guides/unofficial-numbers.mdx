...
<Steps>
  ...
  <Step title="Saiba quando conectou">
    ...
    **Webhook** — pegue o `number.connected` também. Ele é disparado no tratador de conexão do provedor, ou seja, cobre este caminho não-oficial, e chega mesmo com ninguém olhando a sua tela. Três coisas para desenhar em volta dele.
    ...
    <Note>
      **Regra de bolso:** faça polling do `GET /v1/numbers/{id}/status` para conduzir um modal que está na frente da pessoa _e_ como fonte da verdade do estado conectado/desconectado; use o webhook para acordar trabalho quando ninguém estiver com a sua tela aberta.
    </Note>
...
<AccordionGroup>
  <Accordion title="402 ao criar o número — PLAN_NUMBER_LIMIT_REACHED ou INSUFFICIENT_FUNDS">
    A capacidade do plano é consumida **na criação**, antes de a linha ser gravada. As duas recusas são `402`; o `code` é o que diz a saída, e só uma das duas é sobre dinheiro.
