...
<Steps>
  ...
  <Step title="Envie por esse número">
    ...
    O campo `media` só vale no modo de **mídia direta** (nunca junto com `text`) e aceita uma URL http(s) **ou** um data URI base64 (`data:<mime>;base64,…`) de até 50 MiB decodificados com `mediaType: "document"` e 16 MiB nos demais tipos.
