...
<Steps>
  <Step title="Crie o número — e receba o primeiro QR">
    ...
    <Note>
      **O que `linkToApiKey` faz (e o que ele não faz).** Ele só vale quando você chama **com uma chave de número**: nesse caso o padrão é `true` e ele **re-aponta a chave chamadora para o número novo** — ela deixa de estar restrita ao número em que estava antes. Mande `false` para deixá-la onde está. Com uma chave de **tenant** (inclusive uma chave de tenant restrita por `x-whatsapp-number-id`, e OAuth de tenant completo) o campo é **ignorado**: mesmo com `true` nada é vinculado e `linkedApiKeyId` volta `null`.
    ...
    `linkedApiKeyId: null` significa que nenhuma chave foi vinculada nesta chamada. **Significa também que o número novo não tem chave de API própria** — `POST /v1/numbers` nunca cria uma.
  ...
  <Step title="Saiba quando conectou">
    ...
    **Polling** — bom quando a sua tela já está aberta na frente da pessoa:
    ...
    Falha de sincronização devolve `503 { "error", "state" }`. O `state` dessa resposta **não** é leitura ao vivo — é o último valor gravado; só aí podem aparecer `CONNECTING`, `LOGOUT` ou `connecting` em minúsculas. Trate o 503 como "não sei" e continue o polling. Id inexistente devolve `404 { "error": "Not found" }`.
    ...
    <Warning>
      **Um `200 OPEN` também pode vir do último estado conhecido** quando o provedor não responde àquele poll (timeout da sondagem ou provedor inacessível). Isso nunca inventa uma primeira conexão — antes do escaneamento o estado gravado não é `OPEN`, então a mesma falha devolve `503` —, mas significa que um poller de longa duração pode continuar vendo `OPEN` por um tempo depois de uma queda real.
    </Warning>
    ...
    **Webhook** — pegue o `number.connected` também. Ele é disparado no tratador de conexão do provedor, ou seja, cobre este caminho não-oficial, e chega mesmo com ninguém olhando a sua tela. Três coisas para desenhar em volta dele.
    ...
    O `numberId` é o mesmo id que o `POST /v1/numbers` devolveu. O `createdAt` é o momento do disparo do evento, não a data de criação do número. Se o webhook tiver segredo, o corpo vem assinado em `x-pilot-status-signature` (HMAC-SHA256 em hexadecimal). Não existe header `Idempotency-Key` neste evento.
  ...
  <Step title="Envie por esse número">
    ...
    `key` só traz o valor real quando `revealable` é `true`. Vem `null` com `revealable: false` em dois casos: a chave foi criada antes de a criptografia reversível estar ativa (não há o que descriptografar) ou o texto cifrado guardado não descriptografa mais. É um item por chave, não por número — um número que nunca foi usado pode não ter item nenhum, e um número pode ter vários (todos válidos).
    ...
    Como plano B, `POST /v1/api-keys { "whatsappNumberId": "..." }` devolve `{ numberId, keyId, keyPrefix, keyLast4, key, createdAt }`. Se o número ainda não tinha chave, isso apenas **cria** a primeira e devolve o valor utilizável uma única vez.
...
- **Consome capacidade do plano, igual ao `POST /v1/numbers`** — a linha do número passa pela mesma checagem de fundos antes de ser gravada. **Mas o erro chega diferente nesta rota:** acima do teto já custeado e sem como custear, você recebe `500 { "error": "INSUFFICIENT_FUNDS", "code": "INSUFFICIENT_FUNDS" }`, e **não** o `402` do `POST /v1/numbers`. Trate pelo `code`, nunca pelo status.
...
- Aceita `linkToApiKey` e `branding`. Já `redirectUrl` e `externalRef` são consumidos **apenas** pelo ramo Meta. Atenção: o `linkToApiKey` se comporta **diferente** aqui — esta rota vincula mesmo uma chave de tenant e grava apenas o vínculo com a instância.
- `sendViaWhatsApp` é aceito mas **hoje não entrega nada** — `messageSent` volta `false` em toda chamada. Duas coisas precisam se encaixar e a segunda está quebrada hoje: o tenant precisa de um template chamado exatamente `pilot_status_remote_pairing` com uma variável `{{link}}` (se faltar → `messageError: "Template \"pilot_status_remote_pairing\" not found. Create an OTP template with this name and a {{link}} variable."`), e o envio em si falha hoje com `messageError: "WhatsApp instance name is required to enqueue messages"`. Nenhuma das duas falhas é fatal: você ainda recebe **201**, o número e a instância no provedor são criados, o `number.created` dispara e a `remotePairingUrl` é válida. **Entregue o link você mesmo** — mande a `remotePairingUrl` para a pessoa pelo seu próprio canal. E note que o envio sairia do *número mais antigo conectado no seu tenant*, não de um número de plataforma da Pilot Status.
...
<AccordionGroup>
  ...
  <Accordion title="503 no status">
    Falha ao sincronizar o estado com o provedor. A resposta ainda traz `{ error, state }`, mas esse `state` é o **último valor gravado**, não uma leitura ao vivo — é o único lugar em que podem aparecer `CONNECTING`, `LOGOUT` ou `connecting` em minúsculas. Trate como "não sei" e tente novamente na próxima iteração do seu polling.

    **Mas leia o `error` antes de repetir — nem todo 503 é passageiro.** A rota coloca no `error` exatamente o que a sincronização reportou, então uma condição permanente chega com o mesmo status. O caso concreto: `503 { "error": "WhatsApp provider not configured", "state": "CLOSE" }` nunca se resolve sozinho — o provedor daquele número está mal configurado, e um poller que ignora a mensagem fica em loop para sempre. Nesse caso pare o polling e fale com o suporte. Já um 503 com mensagem de timeout é o tipo passageiro — siga tentando.
  </Accordion>
