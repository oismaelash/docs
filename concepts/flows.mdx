...
<Note>
  **Key setup lives on the public API** — see [Flows API](/api/flows). There is no dashboard screen for it yet, and `NAVIGATE` Flows need none of it. Step 1 below is `POST /v1/numbers/{id}/flow-endpoint-key`; steps 2 and 3 are set on the Flow itself.
</Note>
