...
<Warning>
  **Breaking change — `data.numberId` is always the number ID.** On every `number.*` event, `data.numberId` is now the WhatsApp **number** ID (the same `id` returned by `GET /v1/numbers`). `number.created`, `number.connected` and `number.removed` previously carried the internal **instance** ID, while the health events already carried the number ID. **If your consumer matched lifecycle events by instance ID, it will stop matching — key it on the number ID instead.**
</Warning>
