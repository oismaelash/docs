...
A send without it, or with an empty value, is refused with `400 MISSING_TEMPLATE_VARIABLES` (or `INVALID_TEMPLATE_VARIABLE_VALUE`) naming the key. A copy-code button whose example is a named variable (`["{{coupon}}"]`) is still filled by `variables.coupon`, and an authentication (OTP) template's code button still takes the body's code. On unofficial (web/QR) numbers nothing changes: the `example` stored on the button is the code delivered.
