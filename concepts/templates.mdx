...
- **Unofficial EVO\_V2** — the card is **deliverable today** (`review_and_pay`). When the template also carries body text, buttons or a media header, they are delivered first as a normal message and the order card follows as a second message; if the card fails after the first message was delivered, the send is marked failed (no retry, to avoid duplicating the text).
- **Unofficial EVO\_GO** — there is no itemized card; when the order carries a PIX rail it degrades to a "Copiar código Pix" copy button appended to the template's other buttons.
