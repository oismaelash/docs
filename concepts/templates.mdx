...
After saving, `GET /v1/templates/{id}` returns all detected variable keys in the flat top-level `variables` array. For a template created in Meta's console with a numbered button placeholder, the array also lists the button's `button_<N>` key (e.g. `["1", "2", "3", "button_1"]`) — supply every key it lists on send.
