...
To pause deliveries without deleting, `PATCH` with `{ "active": false }`. The signing `secret` is never returned by any endpoint — to validate deliveries with it, see [Verify the signature](/api/webhooks/configure#verify-the-signature).
