...
- Poll [`GET /v1/messages/{id}`](/api/messages/status) for `QUEUED` → `SENT` → `DELIVERED` → `READ` → `PLAYED` (or `FAILED` / `CANCELED`), or
