...
<Note>
  Base64 data URIs are accepted on Meta Cloud API numbers; **unofficial (Pilot Status web)** numbers require a public http(s) URL. Base64 goes up to 50 MiB decoded for a `document` and 16 MiB for every other type — send larger files by URL.
</Note>
