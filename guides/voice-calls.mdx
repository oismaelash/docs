...
<Note>
  ...
  - **Pilot Status web numbers** (unofficial / QR-connected, provider `EVO_GO`) — media is handled **server-side** (SRTP/Opus). **No SDP** is exchanged; you drive audio through the `/play` and `/realtime-session` endpoints instead.

  Any other provider (e.g. Evolution v2) returns `400 FEATURE_NOT_SUPPORTED`. A web number that isn't connected returns `409 WHATSAPP_INSTANCE_NOT_CONNECTED`.
</Note>
...
On a **Meta** number, the dashboard **/chat** page has a built-in browser **softphone** — answer, place, reject, and hang up WhatsApp calls with no WebRTC code. This is the fastest way to try Meta calling. Use the API below when you want to build your own calling experience or when you're on a web number.
...
## Pilot Status web numbers (unofficial / EVO\_GO)
...
# Returns a single-use wsUrl (public Evolution GO host, ~2 min TTL)
...
- **`/realtime-session`** — opens a **full-duplex PCM16 WebSocket** for real-time audio (e.g. an AI voice agent). The returned `wsUrl` lives on the public Evolution GO host, uses a **single-use token** with a **~2-minute TTL**, and you connect to it **directly** — it is not proxied and the HTTP layer does not upgrade the WebSocket.
