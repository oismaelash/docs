...
- The **201** body is `{ provider: "PILOT_STATUS", instance: { id, instanceName, number, displayName, state: "CLOSE" }, remotePairingUrl, maskedNumber, linkedApiKeyId: null }`, plus a `warnings` array when there is something to report (see below). There is **no `expiresAt`** on the QR branch — only the Meta branch returns one.
...
- **It consumes plan capacity, the same as `POST /v1/numbers`** — the number row goes through the same funds check before it is written. Over your funded ceiling and unfundable you get `402 { "error": "INSUFFICIENT_FUNDS", "code": "INSUFFICIENT_FUNDS" }` — the **same status and the same code** as `POST /v1/numbers` (this route used to report it as a `500`).
...
- Other errors: `402 { "error": "INSUFFICIENT_FUNDS", "code": "INSUFFICIENT_FUNDS" }` when a NEW number would exceed your funded ceiling, and `502 { "error": "Failed to create instance", "details": … }` on an upstream failure.
