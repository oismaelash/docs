...
- Other errors: `402 { "error": "INSUFFICIENT_FUNDS", "code": "INSUFFICIENT_FUNDS" }` when a NEW number would exceed your funded ceiling, and `502 { "error": "Failed to create instance" }` on an upstream failure — unlike `POST /v1/numbers`, this route does **not** return a `details` field.
