...
description: "Connect AI agents to WhatsApp via the hosted Pilot Status MCP server: OAuth or x-api-key auth, 93 tools over the public /v1 API, per-number grants."
mode: "wide"
---
...
| Area | Tools |
| --- | --- |
| Messaging | `messages_send`, `message_get`, `messages_list`, `messages_history`, `messages_cancel`, `messages_group`, `messages_unread`, `messages_mark_read`, `messages_typing`, `conversations_list` |
| Ad referrals (CTWA) | `referrals_list`, `referrals_summary` |
...
| Chatwoot history replay | `chatwoot_history_replay`, `chatwoot_history_replay_get`, `chatwoot_history_replay_cancel` |
| Groups | `groups_list`, `groups_create`, `group_info`, `group_participants_add`, `group_participants_remove`, `group_participants_promote`, `group_participants_demote`, `group_invite`, `group_pin` |
| Channels | `newsletters_list` |
| Numbers | `numbers_list`, `numbers_get`, `numbers_create`, `numbers_status`, `numbers_connect`, `numbers_delete`, `numbers_restore`, `numbers_purge`, `numbers_remote_pairing`, `numbers_check`, `numbers_settings_update` |
...
| Voice calls | Meta and web: `call_initiate`, `calls_list`, `call_get`, `call_accept`, `call_reject`, `call_terminate`; Meta only: `call_settings_get`, `call_settings_update`, `call_enable`, `call_permission_get`, `call_permission_request`; Web (unofficial) only: `call_play`, `call_realtime_session` |
| Account | `api_keys_list`, `api_keys_regenerate_number`, `subscription_extra_numbers_get`, `subscription_extra_numbers_add`, `subscription_extra_numbers_remove`, `subscription_extra_numbers_cancel_scheduled`, `billing_checkout`, `branding_get`, `branding_upsert`, `analytics_dashboard`, `media_get` |
| Meta pricing | `meta_pricing_get` |
...
- `numbers_delete` is a 30-day soft delete, undone with `numbers_restore`. `numbers_purge` deletes a number permanently and irreversibly; with an OAuth token only the workspace OWNER may call it.
