...
description: "Connect AI agents to WhatsApp via the hosted Pilot Status MCP server: OAuth or x-api-key auth, 65 tools over the public /v1 API, per-number grants."
mode: "wide"
---
...
| Voice calls | Meta: `call_initiate`, `calls_list`, `call_get`, `call_accept`, `call_reject`, `call_terminate`, `call_settings_get`/`update`, `call_permission_get`/`request`; Web (unofficial): `call_play`, `call_realtime_session` |
