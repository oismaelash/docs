...
- **Coexistence \+ API mirror only.** The button appears only for Meta coexistence numbers with Chatwoot connected in **API mirror (Model A)** mode. A native `whatsapp_cloud` inbox is not eligible.
...
- **On finish**, the touched conversations are marked read and resolved so agents aren't flooded with a burst of notifications.
