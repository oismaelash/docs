...
Button appearance is set **when you mint the link**, inside `branding.button`, and is carried in the signed token. Sending it also puts the link in [button mode](#button-mode) by default, so this one object is the whole request: *style a button, and get a button.*
...
<Warning>
  **Minting the link already costs a plan slot.** `POST /v1/numbers/remote-pairing` creates a placeholder number immediately, and an abandoned link keeps holding that slot — there is no automatic cleanup. Mint the link only when the customer is about to click.
  ...
  When the tenant cannot fund one more number, the call answers **`402`** with `code: "INSUFFICIENT_FUNDS"` — the same body `POST /v1/numbers` returns — and no link and no placeholder are created. Handle it as a billing condition, not as a server fault: this path used to answer `500` for it.
</Warning>
