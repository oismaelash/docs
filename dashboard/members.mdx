...
**Revoke** is the same action as removing a member: one button, because an invitation and a membership are the same row underneath. Pending rows show a _Revoke_ button; active ones show _Remove_.
